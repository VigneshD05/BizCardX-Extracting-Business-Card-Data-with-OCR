...
if __name__ == "__main__":
    ...
    mydb = psycopg2.connect(**config)
    connection_cursor = mydb.cursor()

    # Create the 'bizcardx_db' database if it doesn't exist
    #connection_cursor.execute("CREATE DATABASE IF NOT EXISTS bizcardx_db")

    # Commit the changes and close the connection
    mydb.commit()
    mydb.close()

    # Connect to the 'bizcardx_db' database
    mydb = psycopg2.connect(**config)
    connection_cursor = mydb.cursor()


    # Table creating
    connection_cursor.execute('''CREATE TABLE IF NOT EXISTS card_data
    ...
    if selected == "Existing Card Details":
        ...
        with tab1:
            ...
            config['database'] = 'bizcardx_db'
            mydb = psycopg2.connect(**config)
            initial_cursor = mydb.cursor()
            if st.button("Refresh"):
                initial_cursor.execute("select company_name,card_holder,designation,mobile_number,email,website,area,city,state,pin_code from card_data")
                updated_df = pd.DataFrame(initial_cursor.fetchall(),
